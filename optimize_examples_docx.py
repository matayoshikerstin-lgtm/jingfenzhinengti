...
# 1. 找到“三. 判断方法”和“四. 核心度量指标体系”
start_idx = -1
end_idx = -1

for i, p in enumerate(doc.paragraphs):
    text = p.text.strip()
    if text.startswith('三.') or text.startswith('三、'):
        start_idx = i
...
if start_idx != -1 and end_idx != -1:
    ...
    # 也清空中间可能存在的 table1
    if len(doc.tables) > 1:
        # We can't easily delete table object from python-docx, but we can clear its cells
        t = doc.tables[1]
        t._element.getparent().remove(t._element)
    
    # 我们将新内容插入到 start_idx 段落之后
    target_p = doc.paragraphs[start_idx]
    target_p.text = "三. 自动化打分与判断方法"
...
"""
    ...
    for line in reversed(lines):
        if line.strip() == "":
            current_p.insert_paragraph_before("")
        else:
            p = current_p.insert_paragraph_before(line)
            # 简单排版
...
print("Optimized section 3 examples in docx successfully.")
