...
for i, p in enumerate(doc.paragraphs):
    if p.text.strip().startswith("3. 测试用例"):
        start_idx = i
...
if start_idx != -1 and end_idx != -1:
    ...
    print("Word Document updated with tables.")
...
try:
    ...
    old_section = re.search(r'### 3\. 测试用例与评分展示（典型示例）.*?## 四、', text, re.DOTALL)
    if old_section:
        new_section = """### 3. 测试用例与评分展示（典型示例）
...
| 回复质量 | 智能体回复示例 | LLM 裁判打分逻辑拆解 | 最终得分 |
| :---: | :--- | :--- | :---: |
| 🟢 **优秀** | “目前数智运营产品线赢单率徘徊在 51% 左右，主要痛点在于商务条款分歧以及项目审批延期导致大量商机流失。建议针对延期现象设立阶段停留红线并加强追踪；针对商务难点，方案团队应推出敏捷版/豪华版的梯次化报价策略，以提升最终转化。” | **事实层 (2分)**：覆盖核心数据。<br>**洞察层 (1.5分)**：精准指出延期与条款痛点。<br>**建议层 (1.5分)**：梯次报价与防延期追踪完全具备实操性。 | **5 分** |
...
## 四、"""
        ...
        with open(r'test_plan\经分智能体测试方案2.0.md', 'w', encoding='utf-8') as f:
            f.write(text)
        print("Markdown updated with tables.")
